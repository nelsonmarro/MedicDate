...
public class CitaControllerTest : BaseDbTest
{
  ...
  public static TheoryData<string?, string?> PacienteAndMedicoIds =
    new()
    {
  ...
  public static TheoryData<DateTime, DateTime> FilterDatesForCitas =
    new() { { DateTime.Today, DateTime.Today.AddDays(3) } };

  private readonly List<Actividad> _actividadesCita =
    new()
    {
  ...
  private readonly List<Medico> _medicosCita =
    new()
    {
      new Medico
      {
  ...
  private readonly List<Paciente> _pacientesCita =
    new()
    {
      new Paciente
      {
  ...
  private async Task CreateTestCitaList(ApplicationDbContext context)
  {
    var citas = new List<Cita>
    {
      ...
      new()
      {
        Estado = Sd.ESTADO_CITA_COMPLETADA,
        FechaInicio = DateTime.Now,
  ...
  public class GetCitasByDates : CitaControllerTest
  {
    ...
    )
    {
      ...
      _sut = new CitaController(_citaRepo, _mapper, _citaService);

      //Act
      var citasList = await _sut.GetCitasByDates(
        new CitaByDatesParams { StartDate = startDate, EndDate = endDate }
      );
  ...
  public class GetAllCitasWithPagingAsync : CitaControllerTest
  {
    ...
    public async Task Should_return_the_data_properly_filter_by_the_passed_filter_ids(
      string? pacienteId,
      string? medicoId
    )
    {
      ...
      //Act
      var result = await _sut.GetAllCitasWithPagingAsync(0, 10, medicoId, pacienteId);
