...
public partial class EditCita : ComponentBase
{
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        _actividades =
           await RequestResourceListAsync<ActividadResponseDto>(
              "api/Actividad/listar");
        _pacientes =
           await RequestResourceListAsync<PacienteCitaResponseDto>(
              "api/Paciente/listar");
        _medicos =
           await RequestResourceListAsync<MedicoCitaResponseDto>(
              "api/Medico/listar");

        if (!string.IsNullOrEmpty(Id))
        {
            var httpResp =
               await HttpRepo.Get<CitaRequestDto>($"api/Cita/obtenerParaEditar/{Id}");

            if (!httpResp.Error && httpResp.Response is not null)
            {
                _citaRequest = httpResp.Response;
                if (_citaRequest.ActividadesCita.Count > 0)
                    _actividadesIds = _citaRequest.ActividadesCita
                       .Select(x => x.ActividadId).ToList();
            }
        }
    ...
    {
        var httpResp =
           await HttpRepo.Get<List<ArchivoResponseDto>>(
              $"api/Archivo/listAllByCita/{Id}");

        if (!httpResp.Error && httpResp.Response is not null)
            _loadedArchivosPaciente = httpResp.Response
               .Select(x => new ArchivoCitaVm
               {
    ...
    private bool ValidateDatesForEdit()
    {
        ...
        {
            NotificationService.ShowError("Error!", "Los tiempos de inicio y fin deben estar en el rango de 10:00 - 17:00");
            return false;
        }
        ...
        {
            NotificationService
               .ShowError("Error!", "Los tiempos de inicio y fin de la cita no pueden ser iguales");
            return false;
        }
        ...
        {
            NotificationService
               .ShowError("Error!", "La fecha de inicio debe ser menor que la fecha de finalización");
    ...
    private async Task UpdateCita()
    {
        ...
        if (ValidateDatesForEdit())
        {
            ...
            if (_citaRequest.Estado != Sd.ESTADO_CITA_NOASISTIOPACIENTE
            ...
            var httpResp = await HttpRepo.Put($"api/Cita/editar/{Id}",
               _citaRequest);
    ...
    private async Task SelectActividad(object? value)
    {
        ...
        if (!actividadesIds.Any())
        {
            ...
            if (_actividadesTable is not null &&
                _actividadesTable.DataGridRef is not null)
        ...
        if (_selectedActividades.Count > actividadesIds.Count())
        {
            var actividadesToDelete = _selectedActividades
               .Where(x => !_actividadesIds.Contains(x.ActividadId)).ToList();

            await RemoveActividadFromSelectedListAsync(actividadesToDelete);
        }
        else
        {
            var actividadesIdsToAdd =
               actividadesIds.Except(_selectedActividades.Select(x => x.ActividadId));

            var actividadesToAdd = _actividades?.Select(x =>
    ...
    private async Task AddActividadToSelectedListAsync(
    ...
    private async Task RemoveActividadFromSelectedListAsync(
       ActividadCitaResponseDto actividad)
    {
        _selectedActividades.Remove(actividad);
    ...
    private async Task RemoveActividadFromTableAsync(
       ActividadCitaResponseDto actividad)
    {
    ...
    {
        var archivosCitaRequest = archivosCita.Select(x =>
        ...
        var httpResp =
           await HttpRepo.Post("api/Archivo/subirListado", archivosCitaRequest);
