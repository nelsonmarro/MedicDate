...
namespace MedicDate.Client.Pages
{
  public partial class Index : ComponentBase, IDisposable
  {
    ...
    private readonly string _initialCitasReq =
      $"calendarioCitas?{new DateTime(day: 1, month: DateTime.Today.Month, 
        year: DateTime.Today.Year)}&EndDate={new DateTime(day: 1, month: DateTime.Today.Month,
        year: DateTime.Today.Year).AddMonths(1)}";

    protected override async Task OnInitializedAsync()
    {
      _citasCompletadas = await GetCitaEstadoChartItemListAsync(
        Sd.ESTADO_CITA_COMPLETADA,
        DateTime.Now.Year
      );
      _citasCanceladas = await GetCitaEstadoChartItemListAsync(
        Sd.ESTADO_CITA_CANCELADA,
        DateTime.Now.Year
      );
      _citasNoAsistdas = await GetCitaEstadoChartItemListAsync(
        Sd.ESTADO_CITA_NOASISTIOPACIENTE,
        DateTime.Now.Year
      );

      _citasRegisteredQuarter = await GetCitasAnualQuarterReview(DateTime.Now.Year);

      _pacientesPreviousYear = await GetPacientesAnualMonthReviewAsync(
        DateTime.Now.AddYears(-1).Year
      );
      _pacientesPresentYear = await GetPacientesAnualMonthReviewAsync(DateTime.Now.Year);
    }
    ...
    {
      var httpResp = await HttpRepo.Get<List<T>>($"api/{url}");

      return !httpResp.Error ? httpResp.Response : new List<T>();
    }

    private async Task<List<CitaEstadoMonthReviewDto>?> GetCitaEstadoChartItemListAsync(
      string estado,
      int year
    )
    {
    ...
    private async Task<List<PacienteMonthReviewDto>?> GetPacientesAnualMonthReviewAsync(int year)
    {
    ...
    private async Task<List<CitaRegisteredQuarterReviewDto>?> GetCitasAnualQuarterReview(int year)
    {
