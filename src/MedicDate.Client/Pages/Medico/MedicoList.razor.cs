...
public partial class MedicoList
{
  ...
  private readonly string[] _headers = { "Nombre", "Apellidos", "Cédula", "Email", "Teléfono" };

  private readonly OpRoutes _opRoutes =
    new()
    {
      AddUrl = "medicoCrear",
      EditUrl = "medicoEditar",
      GetUrl = GetUrl
    };
  ...
  {
    var result = await BaseListComponentOps.DeleteItemAndLoadDataList<MedicoResponseDto>(
      id,
      "api/Medico/eliminar",
      GetUrl
    );
  ...
  private async Task FilterByEspecialidad(object value)
  {
    ...
    var result = await BaseListComponentOps.LoadItemListAsync<MedicoResponseDto>(
      GetUrl,
      "&filtrarEspecialidadId=",
      especialidadId
    );
