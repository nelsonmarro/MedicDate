...
public class EspecialidadController : BaseController<Especialidad>
{
   ...
   public EspecialidadController(IEspecialidadRepository especialidadRepo,
     IMapper mapper)
   ...
   public async
     Task<ActionResult<PaginatedResourceListDto<EspecialidadResponseDto>>>
   ...
   [HttpGet("listar")]
   public async Task<ActionResult<List<EspecialidadResponseDto>>>
     GetAllEspecialidadesAsync()
   {
   ...
   public async Task<ActionResult<EspecialidadResponseDto>>
     GetEspecialidadAsync(string id)
   {
      return await GetByIdAsync<EspecialidadResponseDto>(id);
   }

   [HttpGet("obtenerParaEditar/{id}")]
   public async Task<ActionResult<EspecialidadRequestDto>>
     GetPutEspecialidadAsync(string id)
   {
      return await GetByIdAsync<EspecialidadRequestDto>(id);
   }

   [HttpPost("crear")]
   public async Task<ActionResult> CreateEspecialidadAsync(
     EspecialidadRequestDto especialidadResponse)
   {
   ...
   {
      var resp =
        await _especialidadRepo.UpdateEspecialidadAsync(id,
