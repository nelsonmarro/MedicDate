...
[ApiController]
public class MedicoController(IMedicoRepository medicoRepo, IMapper mapper)
  : BaseController<Medico>(medicoRepo, mapper)
{
  private readonly IMapper _mapper = mapper;

  [HttpGet("listarConPaginacion")]
  public async Task<
    ActionResult<PaginatedResourceListDto<MedicoResponseDto>>
  ...
  {
    var includeProperties = traerEspecialidades ? "MedicosEspecialidades.Especialidad" : "";
    ...
    if (!string.IsNullOrEmpty(filtrarEspecialidadId))
      return await GetAllWithPagingAsync<MedicoResponseDto>(
        ...
        pageSize,
        includeProperties,
        m => m.MedicosEspecialidades.Any(me => me.EspecialidadId == filtrarEspecialidadId),
  ...
  {
    return await GetAllAsync<MedicoCitaResponseDto>(x => x.OrderBy(p => p.Nombre));
  }
  ...
  {
    if (await medicoService.ValidatCedulaForCreateAsync(medicoRequestDto.Cedula))
  ...
  {
    if (await medicoService.ValidateCedulaForEditAsync(medicoRequestDto.Cedula, id))
      return BadRequest("Ya existe otro doctor registrado con el número de cédula que ingresó");

    var resp = await medicoRepo.UpdateMedicoAsync(id, medicoRequestDto);
