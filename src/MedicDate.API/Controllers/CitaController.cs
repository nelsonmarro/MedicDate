...
public class CitaController : BaseController<Cita>
{
   ...
   public CitaController(ICitaRepository citaRepo, IMapper mapper) : base(
     citaRepo, mapper)
   {
      _citaRepo = citaRepo;
   }
   ...
   )
   {
      ...
      return await _citaRepo.GetCitasByDateRange(citaByDatesParams.StartDate,
         citaByDatesParams.EndDate, filter);
   }

   [HttpGet("listarConPaginacion")]
   public async Task<ActionResult<PaginatedResourceListDto<CitaCalendarDto>>>
   ...
   {
      return await GetByIdAsync<CitaRequestDto>(id,
         "Medico,Paciente,ActividadesCita.Actividad");
   }

   [HttpGet("getQuarterReview/{requestedYear}")]
   public async Task<ActionResult<List<CitaRegisteredQuarterReviewDto>>>
      GetAnualQuarterReviewAsync([FromServices] ICitaService citaService, int requestedYear)
   {
      return await citaService.GetCitasAnualQuarterReview(requestedYear);
   }

   [HttpGet("getEstadoReview/{requestedYear:int}/{estadoName}")]
   public async Task<ActionResult<List<CitaEstadoMonthReviewDto>>>
      GetCitasEstadoReviewAsync([FromServices] ICitaService citaService,
      string estadoName, int requestedYear)
   {
      return await citaService.GetCitasMonthReviewByEstado(estadoName, requestedYear);
   }

   [HttpPut("actualizarEstado/{id}")]
   public async Task<ActionResult> UpdateEstadoCitaAsync([FromRoute] string id,
   ...
   public async Task<ActionResult> CreateCitaAsync(CitaRequestDto citaRequestDto)
   {
      ...
      return await AddResourceAsync<CitaRequestDto, CitaDetailsDto>(
        citaRequestDto, "GetCitaDetails");
   }

   [HttpPut("editar/{id}")]
   public async Task<ActionResult> UpdateCitaAsync(string id,
