...
namespace MedicDate.Domain.Mapper;
...
public class MapperProfile : Profile
{
  public MapperProfile()
  {
    ...
    CreateMap<Medico, MedicoCitaResponseDto>().ForMember(x => x.FullInfo, opts => opts.Ignore());
    ...
    CreateMap<AppUserRequestDto, ApplicationUser>()
      ...
      .ForMember(x => x.UserRoles, opts => opts.MapFrom(MapUserRoles));
    ...
    CreateMap<Cita, CitaDetailsDto>()
      .ForMember(x => x.ActividadesCita, opts => opts.MapFrom(MapActividadesCitaDetails));

    CreateMap<Cita, CitaRequestDto>()
      .ForMember(x => x.ActividadesCita, opts => opts.MapFrom(MapActividadesCitaForEdit));

    CreateMap<CitaRequestDto, Cita>()
      .ForMember(x => x.Id, opts => opts.Ignore())
      .ForMember(x => x.Paciente, opts => opts.Ignore())
      .ForMember(x => x.Medico, opts => opts.Ignore())
      .ForMember(x => x.ActividadesCita, opts => opts.MapFrom(MapActividadesCitaForCreate));

    CreateMap<Cita, CitaCalendarDto>().ForMember(x => x.InfoCita, opt => opt.Ignore());

    CreateMap<Cita, CitaEmailContent>()
      .ForMember(x => x.DoctorInfo, opts => opts.MapFrom((db, dto) => $"{db.Medico.Nombre} {db.Medico.Apellidos}. Contacto: {db.Medico.PhoneNumber}"))
      .ForMember(x => x.PatientInfo, opts => opts.MapFrom((db, dto) => $"{db.Paciente.Nombres} {db.Paciente.Apellidos}. CI: {db.Paciente.Cedula}"))
      .ForMember(x => x.AppointmentDate, opts => opts.MapFrom((db, dto) => db.FechaInicio.ToString("dddd, d 'de' MMMM 'de' yyyy. h:mm tt", new CultureInfo("es-ES"))))
      .ForMember(x => x.Treatments, opts => opts.MapFrom((db, dto) => db.ActividadesCita.Select(a => a.Actividad.Nombre).ToList()));

    CreateMap<ArchivoDbSaveDto, Archivo>();
  }

  private List<ActividadCita> MapActividadesCitaForCreate(CitaRequestDto citaReqDto, Cita cita)
  {
    var result = new List<ActividadCita>();

    result.AddRange(
      citaReqDto.ActividadesCita.Select(
        x =>
          new ActividadCita
          {
  ...
  )
  {
    ...
    if (cita.ActividadesCita is null || cita.ActividadesCita.First().Actividad is null)
      return result;

    result.AddRange(
      cita.ActividadesCita.Select(
        x =>
  ...
  )
  {
    ...
    result.AddRange(
      applicationUser.UserRoles.Select(
        x =>
          new RoleResponseDto
          {
            Id = x.RoleId,
  ...
  )
  {
    ...
    result.AddRange(appUserRequestDto.Roles.Select(x => new ApplicationUserRole { RoleId = x.Id }));
  ...
  private List<GrupoResponseDto> MapGrupos(
    Paciente paciente,
  ...
  )
  {
    ...
    result.AddRange(
      paciente.GruposPacientes.Select(
        x => new GrupoResponseDto { Id = x.GrupoId, Nombre = x.Grupo.Nombre }
      )
    );
  ...
  )
  {
    ...
    if (pacienteRequestDto.GruposId is null || pacienteRequestDto.GruposId.Count == 0)
      return result;

    result.AddRange(
      pacienteRequestDto.GruposId.Select(grupoId => new GrupoPaciente { GrupoId = grupoId })
    );
  ...
  private List<EspecialidadResponseDto> MapEspecialidades(
    Medico medico,
  ...
  )
  {
    ...
    if (
      medico.MedicosEspecialidades is null
      || medico.MedicosEspecialidades.Any(x => x.Especialidad == null)
    )
      return result;

    result.AddRange(
      medico.MedicosEspecialidades.Select(
        x =>
  ...
  )
  {
    ...
    result.AddRange(
      medicoRequestDto.EspecialidadesId.Select(
